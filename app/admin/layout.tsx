...
export default async function AdminLayout({
  ...
  // Logged in but not the owner — sign out and send to login
  const ownerUuid = process.env.OWNER_UUID;
  if (ownerUuid && user.id !== ownerUuid) {
    ...
    <div className="min-h-screen bg-[var(--color-void)] text-[var(--color-paper)]">
      ...
      <header className="sticky top-0 z-50 flex items-center justify-between border-b border-[var(--color-void-line)] bg-[var(--color-void)]/90 px-6 py-3 backdrop-blur-md">
        ...
        <div className="flex items-center gap-4">
          <Link
            href="/"
            className="text-xs text-[var(--color-muted-ink)] transition-colors hover:text-[var(--color-paper)]"
          >
            View site
          </Link>
